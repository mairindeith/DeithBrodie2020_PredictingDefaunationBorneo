...
resist_infile = os.path.join(infiles,'ResistanceMap','Resistance_hpk_20.Sep.2019_walkingOnly_impassableRivers_Above4.tif')
...
def samplePopDensity(srcfile, shpfile, targetfile, pbuff = 5, settle_filter = None):
    """
    ...
    if settle_filter != None:
        sep = "', '"
        f = "'" + sep.join(settle_filter) + "'"
    # Iteration 2: Villages AND Other, no Longhouses
        lyr.SetAttributeFilter("settleme_1 IN (%s)" %(f))
...
def isochronesToSinks(coordinate_array, multiple, file_path, multiple_number=0):
    """
...
def main():
    print(":::PROGRAM START:::")
    #node_path_all = os.path.join(infiles,'SourceSinks_WalkingOnly_AllNodes','impassableRivers')
    node_path_vs = os.path.join(infiles,'SourceSinks_WalkingOnly_VillageSabah','impassableRivers')
    # node_path_vso = os.path.join(infiles,'SourceSinks_WalkingOnly_VillageSabahOther','impassableRivers')
    
    node_path = [node_path_vs] #, 
    for p in node_path:    
        ensureDir(str(p))
    ...
    filter_count = 0 
    filter_list = [['Village', 'Sabah']] # [['Village','Sabah','Long House', 'Other'], ['Village','Sabah', 'Other'], ['Village','Sabah']] 
    for f in filter_list:
        print("Creating sources filtered by on %s:" %(f))
        sources = samplePopDensity(srcfile, shpfile, targetfile = str(resist_infile), settle_filter = f)
        global node_total
    ...
    #           print(coordinate_set)
                isochronesToSinks(coordinate_set, multiple=False,file_path=str(node_path[filter_count]))
            # Otherwise, if > 50, create multiple files with suffixes for ensuring that .tsv and .txt files are paired
                    ...
                    while current_length < total_length:
                        if current_length + 50 < total_length:
                            ...
                            isochronesToSinks(coordinate_subset, multiple=True, multiple_number=m,file_path=str(node_path[filter_count]))
                            current_length += 50
                            m += 1
                            u_count_start = u_count_current + 1
                            ...
                            u_count_start = u_count_current+1
        print("Completed nodes filtered to %s (saved in %s)") %(f, node_path[filter_count])
        filter_count += 1
