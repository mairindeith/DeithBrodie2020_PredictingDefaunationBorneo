...
for file in $(ls ./SourceSinks/26.Jul.2019_Nodes/NodesTXT/nodes*.txt)[1]; do
	...
	# echo $multiplier
	sources=$(($(awk '{print $2}' ./SourceSinks/26.Jul.2019_Nodes/NodesTSV/$filename.tsv | head -n 1) - 1))
	...
	mpiexec -n 10 /Linux/bin/gflow \
		-habitat ./ResistanceMap/Resistance_hpk_03.Aug.2019.asc \
		-nodes ./SourceSinks/26.Jul.2019_Nodes/NodesTXT/$filename.txt \
		-node_pairs ./SourceSinks/26.Jul.2019_Nodes/NodesTSV/$filename.tsv \
		-effective_resistance ./R_eff.csv \
		-output_sum_density_filename "./GFlowOutputs/Aug2019/Preprocessing/output_temp_{iter}_$filename.asc" \
		-converge_at 3N \
	...
	### output_file_full=$(basename $(ls ./GFlowOutputs/Aug2019/Preprocessing/output_temp*.asc))
	...
	### gdal_calc.py -A GFlowOutputs/Jul2019/Preprocessing/$output_file_full -B GFlowOutputs/Jul2019/FinalOutputs/$final_output --outfile=GFlowOutputs/Jul2019/TmpOutputs/intermediate_cumulative --calc="(A*$adjusted_multiplier*($sources_count/1)/$n_iterations)+B" --overwrite --NoDataValue=-9999 && 
### mv GFlowOutputs/Aug2019/Preprocessing/$output_file_full GFlowOutputs/Aug2019/TmpOutputs/${output_file}_${adjusted_multiplier}_${source_count}.asc && 
### cp GFlowOutputs/Aug2019/TmpOutputs/intermediate_cumulative GFlowOutputs/Aug2019/FinalOutputs/$final_output && 
        mv SourceSinks/26.Jul.2019_Nodes/NodesTSV/$filename.tsv SourceSinks/26.Jul.2019_Nodes/NodesTSV/Processed/$filename.tsv && 
	mv SourceSinks/26.Jul.2019_Nodes/NodesTXT/$filename.txt SourceSinks/26.Jul.2019_Nodes/NodesTXT/Processed/$filename.txt # && rm ../GFlowOutputs/05_03_2018/Temp/${output_file}_${multiplier}.asc
	# else
