...
out_dir = os.path.join(basedir, 'ClusterFlexOutputs/TmpOutputs_Aug05_2019')
asc_dir = os.path.join(out_dir, 'ASCs')
src_dir = os.path.join(basedir, 'SourceSinks/26.Jul.2019_Nodes/NodesTSV')
...
iterpop = [a.replace('output_temp_', '').replace('_nodes_','_').replace('.asc','') for a in asc_list]
...
for d in dup_src:
    ...
    for f in range(len(d[1])):
        ...
        print("......Opening file: %s" %(os.path.basename(asc_tmp)))
        if "aux.xml" in os.path.basename(asc_tmp):
            print(".........skipping.........")
            continue
        with rio.open(asc_tmp, 'r') as ds:
            tmp = ds.read()
